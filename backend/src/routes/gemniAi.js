...
`;
    ...
    try {
        ...
        const result = await pool.query(`
            SELECT name, base_price, description, categories, can_be_served_hot
            FROM product
        ...
        const menuText = result.rows
            .map(
                (item) =>
                    `${item.name} - ${(item.categories || []).join(", ")} - $${item.base_price} - Can be served hot: ${item.can_be_served_hot ? "Yes" : "No"} - ${item.description || "No description available"}`
            )
            .join("\n");
